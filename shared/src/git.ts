...
export class Git {
  ...
  constructor(private extensionHandler: ExtensionHandler, private process: Process) {
    this.initGitURL();
  }

  private async initGitURL(): Promise<void> {
    try {
      const repositoryURL = await this.process.runCommand(this.GIT_COMMAND);
      this.parseURL(repositoryURL);
    } catch (err) {
      const error = err as Error;
      console.log("Error while running command", error);
      this.extensionHandler.showMessage(error.message);
    }
  }
  ...
  private parseURL(repositoryURL: string): Promise<void> | void {
    if (!repositoryURL) {
      return this.extensionHandler.showMessage("Git remote repository not found!");
    }
  ...
  private openHTTPS(repositoryURL: string): Promise<void> {
    ...
    return this.extensionHandler.openURL(url);
  }

  private openSSH(repositoryURL: string): Promise<void> | void {
    if (!(repositoryURL.includes("@") && repositoryURL.includes(":"))) {
      return this.extensionHandler.showMessage("Unknown Git repository!");
    }
    ...
    this.extensionHandler.openURL(url);
  }
}
